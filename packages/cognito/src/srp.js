...
import { toBigInt, fromBigInt, fromBase64, toBase64, fromUtf8, padHex, fromHex, toHex } from './helper/encoding.js';
import concat from 'array-buffer-concat'
import { modPow } from './helper/math.js'
...
// 	const g = BigInt(2);
// 	const kh = await hash('SHA-256', fromHex('00' + N.toString(16) + '0' + g.toString(16)));
// 	const k = toBigInt(kh);
...
// 		const uh = await hash('SHA-256', fromHex(padHex(A) + padHex(B)));
// 		const U = toBigInt(uh);
...
// 		const userPassHex = toHex(userPassHash);

// 		const xh = await hash('SHA-256', fromHex(padHex(S) + padHex(userPassHex)));
// 		const x = toBigInt(xh);
...
// 	const paddedHash = padHex(toHex(hashed));
// 	const salt = padHex(random ? BigInt('0x' + random) : generateRandomBigInt(16).toString(16));

// 	const saltedHash = await hash('SHA-256', fromHex(salt + paddedHash));
...
// 	return hash('SHA-256', stringToArrayBuffer(str));
// }
...
export const srp = async (group, smallAValue = undefined) => {
	...
	return [A, async (user, pass, serverB, salt, secretBlock, time = undefined) => {
		...
		const timestamp = time || getNowString();
		// const timestamp = format(new Date, 'eee LLL d HH:mm:ss z yyyy');
...
export const generateDeviceSecret = () => {
	return toBase64(fromHex(generateRandomBigInt(40).toString(16)));
}
