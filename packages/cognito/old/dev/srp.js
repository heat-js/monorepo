...
export const SRPClient = function (poolName) {
...
SRPClient.prototype = {
	generateRandomSmallA: function () {
		...
		// this.smallAValue = new BigInteger(64, 16).mod(this.N)
		// return this.smallAValue.toString(16);
	},
	calculateA: function (a) {
		// this.smallAValue = a;
		this.smallAValue = a = new BigInteger(a || this.generateRandomSmallA(), 16)
		// console.log('DEV smallA', a.toString(16));
		...
		// console.log('DEV largeA', this.largeAValue.toString(16));
	...
	getPasswordAuthenticationKey: function (username, password, serverBValue, salt) {
		...
		// console.log('DEV U', this.UValue.toString(16));

		var usernamePassword = this.poolName + username + ':' + password
		var usernamePasswordHash = this.hash(usernamePassword)

		// console.log('DEV pass', usernamePasswordHash);
		...
		// console.log('DEV int', intValue2.toString(16));
		...
		// console.log('DEV s', sValue.toString(16));

		var hkdf = this.computehkdf(this.padHex(sValue), this.padHex(this.UValue.toString(16)))

		// console.log('DEV hkdf', sjcl.codec.hex.fromBits(hkdf));
	...
	generateDeviceVerifierConfig: function (deviceGroupKey, username, password, random) {
		const randomPassword = password || sjcl.codec.base64.fromBits(sjcl.random.randomWords(8, 10), 0);
		const combinedString = `${deviceGroupKey}${username}:${randomPassword}`;
		const hashedString = this.hash(combinedString);

		// console.log('paddedHash', hashedString);

		const hexRandom = new BigInteger(random || sjcl.codec.hex.fromBits(sjcl.random.randomWords(8, 0)), 16);
		...
		// const saltedHash = await hash('SHA-256', bigIntToArrayBuffer(BigInt('0x' + salt + paddedHash)));
		// console.log('salt', SaltToHashDevices);
		// console.log('hash', this.hexHash(SaltToHashDevices + hashedString));
		// console.log('verifier', deviceVerifier.toString(16));
...
export const calculateSignature = function (hkdf, userPoolId, username, secretBlock, dateNow) {

	// console.log(userPoolId, username, secretBlock, dateNow);

	// let msg = sjcl.codec.utf8String.toBits(userPoolId);
	// msg = sjcl.bitArray.concat(msg, sjcl.codec.utf8String.toBits(username));
	// msg = sjcl.bitArray.concat(msg, sjcl.codec.base64.toBits(secretBlock));
	// msg = sjcl.bitArray.concat(msg, sjcl.codec.utf8String.toBits(dateNow));

	// console.log('DEV message', sjcl.codec.hex.fromBits(msg));
	...
	// console.log('DEV sig hex', sjcl.codec.hex.fromBits(mac.digest()));
	return sjcl.codec.base64.fromBits(mac.digest())
}

export const getNowString = function () {
	var now = new Date()
...
// export const SRPClient = SRPClient;
// export const calculateSignature = calculateSignature;
// export const getNowString = getNowString;
//
